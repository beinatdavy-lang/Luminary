...
function openCaptureMulti(items) {
  ...
  if (saved > 0) {
    persist();
    renderView(curView);
    showToast('✦ ' + saved + ' highlight' + (saved > 1 ? 's' : '') + ' sauvegardé' + (saved > 1 ? 's' : '') + ' !');
  }
}
...
function renderPages() {
  ...
  if (!g) return;
  if (!PAGES.length) { g.innerHTML = ''; if (e) e.style.display = ''; return; }
...
window.addEventListener('message', function(e) {
  ...
  var page = {
    ...
    url:        data.url || '',
    content:    (data.content || '').substring(0, 200000),
    date:       new Date().toISOString(),
    highlights: []
  };
  PAGES.unshift(page);
  persist();
  if (e.source) { try { e.source.postMessage({ type: 'rl-ack', pageId: page.id }, '*'); } catch(err) {} }
  curPage = page;
  go('reader');
  showToast('Page sauvegardee - selectionnez pour surligner');
});
